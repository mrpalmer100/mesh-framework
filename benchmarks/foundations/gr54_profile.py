...
CKPT = pathlib.Path('/tmp/gr54_ckpt.pkl')
DUR = ROOT / 'analysis' / 'gr54_ckpt.pkl'
...
def main():
    ...
    if st['gr54|s0'].get('refused') or st['gr54|s1'].get('refused'):
        print("[gr54] seed continuation refused -- GR-OPEN path; "
              "stop and record.", flush=True)
        return

    # ---- Phase 2: arc profile at 54
    rec = st.setdefault('gr54prof', dict(states=[], meas=[]))
    if not rec['states']:
        rec['states'] = [np.asarray(st['gr54|s0']['x'], float),
                         np.asarray(st['gr54|s1']['x'], float)]
        st['gr54prof'] = rec
    if len(rec['meas']) >= BUDGET:
        print("[gr54] PROFILE COMPLETE -- run the verdict "
              "ONCE.", flush=True)
        return
    i = len(rec['meas'])
    ...
    xn = SJ.gn_sparse(T54, xb + DS * t, 'arc', (xb, t, DS), sj, bs,
                      rounds=60, st=st, key=f'gr54|p{i}|solve')
    r = T54.field_rms(xn)
    clos = T54.closure_max(xn)
    if not (r < q1.RMS_BAR and clos < q1.CLOSURE_BAR):
        cum = st.get(f'gr54|p{i}|solve-cum', 0)
        if cum >= 60:
            rec['halt'] = f'point {i} refused (RMS {r:.1e})'
            st['gr54prof'] = rec
            DUR.write_bytes(pickle.dumps(dict(st)))
            print(f"[gr54 p{i}] ** REFUSED at budget ** recorded.",
                  flush=True)
        else:
            print(f"[gr54 p{i}] not gated (cum {cum}) -- resume",
                  flush=True)
        return
    ...
    st['gr54prof'] = rec
    DUR.write_bytes(pickle.dumps(dict(st)))
    print(f"[gr54 p{i}] GATED  A2 {m['A2']:.7f}  (measurements "
          f"sealed)", flush=True)
