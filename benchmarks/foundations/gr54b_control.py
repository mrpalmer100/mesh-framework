"""GR54-B -- 5/3 flat-cell control at 144x54 (charter
analysis/GR54B_charter_LOCKED.md). GR54 driver, cell 5/3.
...
CKPT = pathlib.Path('/tmp/gr54b_ckpt.pkl')
DUR = ROOT / 'analysis' / 'gr54b_ckpt.pkl'
...
def main():
    ...
    S36 = prof['prof-5/3']['states']
    T36 = q1.QTGrid(144, 36, 3, 5)
    T54 = q1.QTGrid(144, 54, 3, 5)
    ...
    if st['gr54b|s0'].get('refused') or st['gr54b|s1'].get('refused'):
        print("[gr54b] seed continuation refused -- GR-OPEN path; "
              "stop and record.", flush=True)
        return

    # ---- Phase 2: arc profile at 54
    rec = st.setdefault('gr54bprof', dict(states=[], meas=[]))
    if not rec['states']:
        rec['states'] = [np.asarray(st['gr54b|s0']['x'], float),
                         np.asarray(st['gr54b|s1']['x'], float)]
        st['gr54bprof'] = rec
    if len(rec['meas']) >= BUDGET:
        print("[gr54b] PROFILE COMPLETE -- run the verdict "
              "ONCE.", flush=True)
        return
    i = len(rec['meas'])
    ...
    xn = SJ.gn_sparse(T54, xb + DS * t, 'arc', (xb, t, DS), sj, bs,
                      rounds=60, st=st, key=f'gr54b|p{i}|solve')
    r = T54.field_rms(xn)
    clos = T54.closure_max(xn)
    if not (r < q1.RMS_BAR and clos < q1.CLOSURE_BAR):
        cum = st.get(f'gr54b|p{i}|solve-cum', 0)
        if cum >= 60:
            rec['halt'] = f'point {i} refused (RMS {r:.1e})'
            st['gr54bprof'] = rec
            DUR.write_bytes(pickle.dumps(dict(st)))
            print(f"[gr54b p{i}] ** REFUSED at budget ** recorded.",
                  flush=True)
        else:
            print(f"[gr54b p{i}] not gated (cum {cum}) -- resume",
                  flush=True)
        return
    ...
    st['gr54bprof'] = rec
    DUR.write_bytes(pickle.dumps(dict(st)))
    print(f"[gr54b p{i}] GATED  A2 {m['A2']:.7f}  (measurements "
          f"sealed)", flush=True)
